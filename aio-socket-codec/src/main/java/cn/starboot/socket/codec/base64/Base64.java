...
/**
 * Utility class for {@link java.nio.ByteBuffer} that encodes and decodes to and from
...
public final class Base64 {
	...
	private static byte[] alphabet(Base64Dialect dialect) {
		return ObjUtils.checkNotNull(dialect, "dialect").alphabet;
	}

	private static byte[] decodabet(Base64Dialect dialect) {
		return ObjUtils.checkNotNull(dialect, "dialect").decodabet;
	}

	private static boolean breakLines(Base64Dialect dialect) {
		return ObjUtils.checkNotNull(dialect, "dialect").breakLinesByDefault;
	}


	public static void encode(Base64Packet base64Packet, ChannelContext channelContext) throws AioEncoderException {
		encode(base64Packet, channelContext, Base64Dialect.STANDARD);
	}

	public static void encode(Base64Packet base64Packet, ChannelContext channelContext, Base64Dialect dialect) throws AioEncoderException {
		encode(base64Packet, channelContext, breakLines(dialect), dialect);
	}

	public static void encode(Base64Packet base64Packet, ChannelContext channelContext, boolean breakLines) throws AioEncoderException {
		encode(base64Packet, channelContext, breakLines, Base64Dialect.STANDARD);
	}

	public static void encode(Base64Packet base64Packet, ChannelContext channelContext, boolean breakLines, Base64Dialect dialect) throws AioEncoderException {
		ObjUtils.checkNotNull(base64Packet, "base64Packet");
		encode(base64Packet, 0, base64Packet.getLength(), channelContext, breakLines, dialect);
	}

	public static void encode(Base64Packet base64Packet, int off, int len, ChannelContext channelContext) throws AioEncoderException {
		encode(base64Packet, off, len, channelContext, Base64Dialect.STANDARD);
	}

	public static void encode(Base64Packet base64Packet, int off, int len, ChannelContext channelContext, Base64Dialect dialect) throws AioEncoderException {
		encode(base64Packet, off, len, channelContext, breakLines(dialect), dialect);
	}

	public static void encode(Base64Packet base64Packet,
							  int off,
							  int len,
							  ChannelContext channelContext,
							  boolean breakLines) throws AioEncoderException {
		encode(base64Packet, off, len, channelContext, breakLines, Base64Dialect.STANDARD);
	}


	public static void encode(
			Base64Packet base64Packet, int off, int len, ChannelContext channelContext, boolean breakLines, Base64Dialect dialect) throws AioEncoderException {
		ObjUtils.checkNotNull(base64Packet, "base64Packet");
		ObjUtils.checkNotNull(base64Packet.getData(), "base64PacketData");
		ObjUtils.checkNotNull(dialect, "dialect");
		ObjUtils.checkNotNull(channelContext, "channelContext");

		ByteBuffer wrap = ByteBuffer.wrap(base64Packet.getData());
...
//		ByteBuf dest = allocator.buffer(encodedBufferSize(len, breakLines)).order(src.order());
		...
		int lineLength = 0;
		for (; d < len2; d += 3, e += 4) {
			encode3to4(wrap, d + off, 3, writeBuffer, alphabet);

			lineLength += 4;

			if (breakLines && lineLength == MAX_LINE_LENGTH) {
				writeBuffer.write(NEW_LINE);
				e ++;
				lineLength = 0;
			} // end if: end of line
		...
		if (d < len) {
			encode3to4(wrap, d + off, len - d, writeBuffer, alphabet);
			e += 4;
		} // end if: some padding needed

		// Remove last byte if it's a newline
//		if (e > 1 && writeBuffer.get(e - 1) == NEW_LINE) {
//			e--;
//		}
	}
	...
	private static void encode3to4(
			ByteBuffer src, int srcOffset, int numSigBytes, WriteBuffer writeBuffer, byte[] alphabet) throws AioEncoderException {
		//           1         2         3
		...
		if (src.order() == ByteOrder.BIG_ENDIAN) {
			...
			switch (numSigBytes) {
				...
				case 2:
					...
					break;
				default:
					inBuff = numSigBytes <= 0 ? 0 : toIntBE(src.get(srcOffset));
					break;
			}
			encode3to4BigEndian(inBuff, numSigBytes, writeBuffer, alphabet);
		} else {
			final int inBuff;
			switch (numSigBytes) {
				case 1:
				...
				case 2:
					...
					break;
				default:
					inBuff = numSigBytes <= 0 ? 0 : toIntLE(src.get(srcOffset));
					break;
			}
			encode3to4LittleEndian(inBuff, numSigBytes, writeBuffer, alphabet);
		}
	}
	...
	private static void encode3to4BigEndian(
			int inBuff, int numSigBytes, WriteBuffer writeBuffer, byte[] alphabet) throws AioEncoderException {
		// Packing bytes into an int to reduce bound and reference count checking.
		switch (numSigBytes) {
			case 3:
				writeBuffer.writeInt(alphabet[inBuff >>> 18       ] << 24 |
	...
	private static void encode3to4LittleEndian(
		...
		switch (numSigBytes) {
			case 3:
				writeBuffer.writeInt(alphabet[inBuff >>> 18       ]       |
	...
	public static Packet decode(ByteBuffer src) {
		return decode(src, Base64Dialect.STANDARD);
	}

	public static Packet decode(ByteBuffer src, Base64Dialect dialect) {
		ObjUtils.checkNotNull(src, "src");
		return decode(src, src.position(), src.remaining(), dialect);
	}

	public static Packet decode(
			ByteBuffer src, int off, int len) {
		return decode(src, off, len, Base64Dialect.STANDARD);
	}

	public static Packet decode(
			ByteBuffer src, int off, int len, Base64Dialect dialect) {
		ObjUtils.checkNotNull(src, "src");
		ObjUtils.checkNotNull(dialect, "dialect");

		// Using a ByteProcessor to reduce bound and reference count checking.
		return new Decoder().decode(src, off, len, dialect);
	}
	...
	private static final class Decoder {
		...
		Packet decode(ByteBuffer src, int off, int len, Base64Dialect dialect) {
...
//				PlatformDependent.throwException(cause);
		...
		private static int decode4to3(byte[] src, Packet dest, int destOffset, byte[] decodabet) {
...
//				dest.setByte(destOffset, decodedValue);
			...
			if (src3 == EQUALS_SIGN) {
				...
				try {
					if (bigEndian == ByteOrder.BIG_ENDIAN) {
