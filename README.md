...
aio-socket是Java AIO通讯内核，采用了内存池、线程池、对象池(开发中)、缓存池以及插件化增添模块等思想进行设计。
...
致传输层Java通讯框架小白：<br/>
一、aio-socket并不是凭空而来，也不是多么优秀的设计。
...
二、其实我们所追求的所谓高性能通讯，只不过是通过多线程尽可能的使网络100%占用率。让通讯服务器的瓶颈发生在带宽上。
与此同时CPU最低占用率，让给应用程序更多地计算资源。

三、在研究所有语言的传输层通讯框架时，需要对计算机操作系统和计算机网络了解。
因为，所有优秀的框架最核心的东西就是书上的思想。它之所以被称为计算机四大天书肯定是有其原因。
